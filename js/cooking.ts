class Post {
  ...
  constructor(day: number, month: number, year: number, image: string, label: number, contents: string[] = ["[Contents]"], urls: string[] = [], time: string | null = null, title: string | null = null, description: string | null = null) {
    this.date = `${day} ${Post.monthArr[month - 1]} 20${year}`;
    this.image = `https://drive.google.com/uc?export=view&id=${image}`;
    this.label = label;
  ...
  private header(): HTMLDivElement {
    ...
    const date = document.createTextNode(this.date);
  ...
  private extraBlock(): HTMLDivElement {
    ...
    for (let i = 0; i < Post.labelArr.length; i++)
      if ((this.label & 1 << i) > 0) {
        ...
        if ((tableFilter & 1 << i) > 0) {
          ...
          label.onclick = () => filterTable(tableFilter - (1 << i));
        } else {
    ...
    if (this.time != null) {
      ...
      const text = document.createTextNode(this.time);
...
const tableVolume: number = 10; // number of posts per page
const tableSpan: number = 2; // number of navigable pages

var postContainer: HTMLElement;
var postNavigation: HTMLElement;
var tableContents: Post[] = posts;
var tableFilter: number = 0;
var tableMaxPage: number;
var tableCurPage: number = 0;

function setupTable(container: HTMLElement, navigation: HTMLElement): void {
  postContainer = container;
  postNavigation = navigation;
}

function filterTable(filter: number): void {
  if (postContainer == null) return; // checks if container is applicable for filters
  tableCurPage = 0;
  tableFilter = filter;
  tableContents = posts.filter(p => (p.label & filter) == filter);
  reloadTable();
}
...
function loadTable(page: number): void {
  page = Math.min(page, tableMaxPage); // validate
  if (page == tableCurPage) return;
  console.log("loading");
  ...
  function resetNavigation(page: number): void {
    ...
    if (page - tableSpan > 2) addNavigation(0, list);
    for (let i = Math.max(2, page - tableSpan); i <= Math.min(tableMaxPage - 1, page + tableSpan); i++) addNavigation(i, list);
    if (page + tableSpan < tableMaxPage - 1) addNavigation(0, list);
    if (tableMaxPage > 1) addNavigation(tableMaxPage, list);
    ...
    function goto(): HTMLElement {
      ...
      input.onkeyup = (e) => {
        if (e.key == "Enter") {
          ...
          if (!isNaN(page) && page > 0) loadTable(page);
          input.value = "";
        }
      }
  ...
  const volume = Math.min(tableVolume, tableContents.length - (page - 1) * tableVolume);
  const start = (page - 1) * tableVolume;
